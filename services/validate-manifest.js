...
function validateManifest(config, opts = {}) {
  ...
  // that binding. Fail loud on a bad shape, matching how ticketSource rejects a typo.
  if (config.tmuxPrefix != null && !(isNonEmptyString(config.tmuxPrefix) && !/\s/.test(config.tmuxPrefix))) {
    errors.push(
      `"tmuxPrefix" must be a non-empty string with no whitespace (tmux key notation, e.g. "C-a"), got ${JSON.stringify(config.tmuxPrefix)}`
    );
  }
