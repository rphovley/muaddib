...
function assertRoute(labels, expectedEntryPoint) {
  ...
  if (expectedEntryPoint === null) {
    if (route !== null) throw new Error(`expected null, got entryPoint=${route.entryPoint}`);
  } else {
    if (!route) throw new Error(`expected route with entryPoint=${expectedEntryPoint}, got null`);
    if (route.entryPoint !== expectedEntryPoint) {
      throw new Error(`expected entryPoint=${expectedEntryPoint}, got ${route.entryPoint}`);
    }
  }
}
...
async function testBugLabel() {
  ...
  if (!route.workflowFile.endsWith('bug.json')) throw new Error(`expected bug.json, got ${route.workflowFile}`);
}
...
async function testFastLabel() {
  ...
  if (!route.workflowFile.endsWith('feature-fast.json')) throw new Error(`expected feature-fast.json, got ${route.workflowFile}`);
}
...
async function testPlanOnlyLabel() {
  ...
  if (!route.workflowFile.endsWith('plan.json')) throw new Error(`expected plan.json, got ${route.workflowFile}`);
}
...
async function testBugTakesPrecedence() {
  ...
  if (!route.workflowFile.endsWith('bug.json')) throw new Error(`expected bug.json (bug before fast), got ${route.workflowFile}`);
}
...
async function testLabelsCaseInsensitive() {
  ...
  if (!bugRoute.workflowFile.endsWith('feature.json')) {
    throw new Error(`expected feature.json for un-lowercased 'Bug', got ${bugRoute.workflowFile}`);
  }
  ...
  if (!normalRoute.workflowFile.endsWith('bug.json')) {
    throw new Error(`expected bug.json for lowercased 'bug', got ${normalRoute.workflowFile}`);
  }
}
...
const REAL_UPDATE_BODY = Buffer.from(JSON.stringify({
  action: 'update',
  type: 'Issue',
  data: {
...
async function testRealWebhookAutoLabel() {
  ...
  if (logged.includes('no route matched')) {
    throw new Error(`label extraction failed — got "no route matched": ${logged}`);
  }
  // Should NOT log "update without label change" or "labels unchanged"
  if (logged.includes('update without label change') || logged.includes('labels unchanged')) {
...
async function testRealWebhookNoLabels() {
  ...
  const body = Buffer.from(JSON.stringify({
    ...
    updatedFrom: { labelIds: ['c328c424-c60e-4c60-8f64-2ba5282e6090'], updatedAt: '2026-06-07T00:00:00.000Z' },
  }));
  const lines = [];
  const origWrite = process.stdout.write.bind(process.stdout);
  process.stdout.write = (s) => { lines.push(s); return true; };
  ...
  const logged = lines.join('');
  if (!logged.includes('no route matched')) {
    throw new Error(`expected "no route matched" for empty labels, got: ${logged}`);
  }
}
...
async function main() {
  const tests = [
    ...
    ['resolveRoute: auto + muaddib:fast → feature-fast workflow', testMuaddibFastLabel],
