---
name: gather-context
description: Fleet context-gathering phase. Loads a Linear ticket and its plan comment, branches from main, then writes branch and ticket_url to worker state. Called before the implement step in the feature workflow.
---

# Gather Context

Fleet-safe first step of the feature workflow. **Never calls `AskUserQuestion`.**
...
If no plan comment is found on this ticket and `parentId` is set, call `mcp__linear__get_issue` and `mcp__linear__list_comments` on the parent and look there.
...
`<short-slug>` is 2–4 kebab-case words from the ticket title. Example: `quo-318-definition-driven-orchestration`.
